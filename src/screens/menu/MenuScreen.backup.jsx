...
const MenuScreen = ({ navigation }) => {
  ...
  const categoryScrollRef = useRef(null); // For horizontal category scroll
  const categoryRefs = useRef({});
  const categoryPositions = useRef({}); // Store Y positions of each category
  const categoryTabRefs = useRef({}); // Store refs for each category tab
  const isScrollingProgrammatically = useRef(false); // Track manual scrolls
  ...
  const handleScroll = (event) => {
    // Don't update category if user manually clicked a tab
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
                        ...
                        <View style={styles.buttonOverlay}>
                          {quantity === 0 ? (
                            <TouchableOpacity
                              ...
                              onPress={(e) => {
                                e.stopPropagation();
                                handleItemPress(item); // Open modal instead of adding directly
                              }}
                              activeOpacity={0.7}
                            >
      ...
      {/* View Cart Button - Like Start Your Order */}
      {cartTotal > 0 && (
        <View style={styles.bottomBar}>
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text style={styles.itemsCount}>{cartTotal} {cartTotal === 1 ? 'item' : 'items'}</Text>
...
const styles = StyleSheet.create({
  ...
  bottomSpacing: {
    height: 150, // Increased to clear View Cart button + tab bar
  },
  bottomBar: {
    position: 'absolute',
    bottom: 0,
    ...
    paddingBottom: 100, // Increased to clear tab bar (90px tabs + 10px extra)
  },
  viewCartButton: {
